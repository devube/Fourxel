...
void Sampler::setMagnificationFilter(const MagnificationFilter magnificationFiler) const {
    if (!createdCheck()) {
        return;
    }
    ...
    if (magnificationFiler == MAG_FILTER_BILINEAR) {
        param = GL_LINEAR;
    }
...
bool Sampler::createdCheck() const {
    if (!_isCreated) {
        std::cout << "Attempting to access non existing sampler!" << '\n';
        return false;
    }
